...
# ChangeLog 2015-03-12 
#   1. fixed a bug where length_seq is not set in the scoring function below
#
#   2. changed the scoring function from 
#      score = queue_time_in_sec / ((rank+addition)**1.5 * length_seq)
#   to
#      score = queue_time_in_sec / ((rank+addition)**2 * max(length_seq,100)**1.5)

usage_short="""
Usage: %s -resultdir DIR [-o OUTFILE]
"""%(sys.argv[0])
...
Created 2014-12-02, updated 2015-03-12, Nanjiang Shu
"""
usage_exp="""
Examples:
    %s -resultdir result -o jobqueue.txt
"""
...
# 1. get the list of working folders
    ...
    for folder in raw_folder_list:
        if os.path.isdir(resultdir+"/"+folder) and (folder.isdigit() or folder[0:2] == "r_"):
...
# 2. gather information for queued jobs
    ...
    for folder in folder_nr_list:
        ...
        if status in ["Queued"]: # jobs in queue
            ...
            # temporary dealing with the special case
            if email == "majianzhu@gmail.com" and date_str.find("2014") == 0:
                queue_time_in_sec = 10.0
...
# where addition = freq_running
    for user in freq_user_in_queue:
        ...
        for i in xrange(len(sorted_sub_table)):
            ...
            score = queue_time_in_sec/((rank+addition)**2 * max(length_seq, 50)**2)
